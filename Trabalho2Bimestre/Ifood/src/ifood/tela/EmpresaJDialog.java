 ...
 */
public class EmpresaJDialog extends javax.swing.JDialog {
    
    EmpresaDAO empresaDao;

    /**
     * Creates new form EmpresaJDialog
     */
    public EmpresaJDialog(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        empresaDao = new EmpresaDAO();
        setTfCodigo();
        try {
            carregaTable(empresaDao.getAll());
        } catch (SQLException ex) {
            Logger.getLogger(EmpresaJDialog.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    private void initComponents() {
        ...
        tbEmpresa.setFont(new java.awt.Font("Comic Sans MS", 1, 12)); // NOI18N
        ...
        pnTabela.setViewportView(tbEmpresa);
        ...
        layout.setVerticalGroup(
            ...
            .addGroup(javax.swing.GroupLayout.Alignment.TRAILING, layout.createSequentialGroup()
                ...
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING)
                    ...
                    .addComponent(pnTabela, javax.swing.GroupLayout.DEFAULT_SIZE, 369, Short.MAX_VALUE))
                .addGap(21, 21, 21))
        );
    ...
    private void tbEmpresaMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tbEmpresaMouseClicked
        ...
        try {
            empresa = empresaDao.getById((int) tbEmpresa.getValueAt(tbEmpresa.getSelectedRow(), 0));
        } catch (SQLException ex) {
            ex.printStackTrace();
        }
        novo();
        btSalvar.setText("Atualizar");
        tfCodigo.setText(String.valueOf(empresa.getId()));
        tfCnpj.setText(empresa.getCnpj());
        tfNome.setText(empresa.getNome());
        tfEndereco.setText(empresa.getEndereco());
        tfTelefone.setText(String.valueOf(empresa.getTelefone()));
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(EmpresaJDialog.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
    ...
    private void carregaTable(List<Empresa> empresaList) {
        if (empresaList == null)
            return;
        DefaultTableModel model = (DefaultTableModel) tbEmpresa.getModel();
        model.setRowCount(0);
        for (Empresa e : empresaList) {
            model.addRow(new Object[]{e.getId(), e.getNome(), e.getCnpj()});
        }
    }

    private void setTfCodigo() {
        try {
            tfCodigo.setText(String.valueOf(empresaDao.getLastId()));
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    }
    ...
    private void desabilitaCampos(boolean ativo) {
        ...
        tfCnpj.setEnabled(ativo);
        tfTelefone.setEnabled(ativo);
        tfIe.setEnabled(ativo);
    }
    ...
    private boolean validaCampos() {
        if (tfCodigo.getText().trim().length() <= 0) {
            ...
            JOptionPane.showMessageDialog(null, "Informe um telefone para salvar.", "Atenção",
                    JOptionPane.ERROR_MESSAGE);
            return false;
        } else if (tfCnpj.getText().trim().length() <= 0) {
            JOptionPane.showMessageDialog(null, "Informe um CNPJ para salvar.", "Atenção",
                    JOptionPane.ERROR_MESSAGE);
            return false;
            ...
            JOptionPane.showMessageDialog(null, "Informe um endereço para salvar.", "Atenção",
                    JOptionPane.ERROR_MESSAGE);
            return false;
        } else if (tfIe.getText().trim().length() <= 0) {
            JOptionPane.showMessageDialog(null, "Informe uma IE para salvar.", "Atenção",
                    JOptionPane.ERROR_MESSAGE);
            return false;
        }
        return true;
    ...
    private void salvar() {
        try {
            Empresa empresa = new Empresa();
            empresa.setId(Integer.parseInt(tfCodigo.getText().trim()));
            empresa.setCnpj(tfCnpj.getText().trim());
            empresa.setEndereco(tfEndereco.getText().trim());
            empresa.setNome(tfNome.getText().trim());
            empresa.setTelefone(Integer.parseInt(tfTelefone.getText().trim()));
            empresa.setIe(tfIe.getText().trim());
            try {
                if (empresaDao.getLastId() == Integer.parseInt(tfCodigo.getText())) {
    ...
    private void remover() {
        int linhaSelecionada = tbEmpresa.getSelectedRow();
        if (linhaSelecionada == -1) {
            JOptionPane.showMessageDialog(null, "Deve-se selecionar uma linha para ser removido", "Atenção", JOptionPane.ERROR_MESSAGE);
            return;
        }
        int codigoRemover = (int) tbEmpresa.getValueAt(linhaSelecionada, 0);
        try {
            empresaDao.delete(codigoRemover);
    ...
    private void filtrar() {
        try {
            if (rbCodigo.isSelected() && tfFiltroCodigo.getText().trim().length() > 0) {
                Empresa empresa = empresaDao.getById(Integer.parseInt(tfFiltroCodigo.getText()));
                List<Empresa> empresaList = new ArrayList<>();
                empresaList.add(empresa);
                carregaTable(empresaList);
            } else if (rbNome.isSelected() && tfFiltroNome.getText().trim().length() > 0) {
                carregaTable(empresaDao.getByName(tfFiltroNome.getText()));
            } else {
